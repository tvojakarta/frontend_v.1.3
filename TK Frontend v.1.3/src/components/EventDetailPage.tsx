...
import { Calendar, MapPin, Clock, Users, ArrowLeft, Plus, Minus, ShoppingCart, CreditCard, UserPlus } from 'lucide-react';
...
export function EventDetailPage({ eventId, onNavigate }: EventDetailPageProps) {
  ...
  const continueToPayment = () => {
    ...
    if (foundEvent?.isFree) {
      // For free events, navigate to reservation page
      if (getTotalSelected() > 0) {
        onNavigate('reservation', event.id);
      }
      return;
    }
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2">
          <div className="mb-6">
            ...
            <div className="text-xl text-muted-foreground mb-6 whitespace-pre-line">
              {event.localizedDescription.split(/(kontakt stranici|contact page)/).map((part, index) => {
                if (part === 'kontakt stranici' || part === 'contact page') {
                  return (
                    <button 
                      key={index}
        ...
        {/* Ticket Selection / Registration */}
        <div className="lg:col-span-1">
          <Card className="sticky top-8">
            <CardHeader>
              <CardTitle>
                {foundEvent?.isFree ? t('reservation.title') : t('event.select_ticket')}
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-6">
              {foundEvent?.isFree ? (
                ...
                <div className="space-y-6">
                  {registrationSuccess ? (
                    <div className="text-center p-6 bg-green-50 rounded-lg border border-green-200">
                      ...
                      <p className="text-green-800 leading-relaxed">{t('reservation.success_message')}</p>
                    </div>
                  ) : (
                    <>
                      <div className="bg-gradient-to-br from-green-50 to-emerald-50 border border-green-200 rounded-lg p-5">
                        <div className="flex items-center justify-center gap-2 mb-3">
                          <Badge className="bg-green-600 hover:bg-green-700 text-white px-3 py-1">
                            {t('reservation.free_event')}
                          </Badge>
                        </div>
                        <div className="text-center">
                          <h3 className="mb-2">{event.localizedTicketTypes[0]?.name || (language === 'sr' ? 'Ulaznica' : 'Ticket')}</h3>
                          <p className="text-sm text-muted-foreground mb-2">
                            {event.localizedTicketTypes[0]?.description || (language === 'sr' ? 'Besplatan pristup događaju' : 'Free access to the event')}
                          </p>
                          <div className="flex items-center justify-center gap-2 text-sm text-muted-foreground">
                            <Users className="h-4 w-4" />
                            <span>{event.localizedTicketTypes[0]?.available || event.ticketsAvailable} {t('event.available')}</span>
                          </div>
                        </div>
                      </div>

                      <div className="space-y-2">
                        <Label htmlFor="registration-email" className="text-base">{t('reservation.form.email')}</Label>
                        <Input
                ...
                <>
                    ...
                    <div key={ticketType.id} className="border rounded-lg p-4">
                      <div className="flex justify-between items-start mb-3">
                        ...
                        <span className="text-xl">
                          {ticketType.price} {t('common.currency')}
                        </span>
                      </div>
                  ...
                  {/* Action Buttons for Paid Events */}
                  <div className="space-y-2">
                    <Button 
                      className="w-full"
