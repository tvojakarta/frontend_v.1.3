      ...
      {filteredEvents.length === 0 && (
        <>
          {activeCategory && activeCategory !== 'all' ? (
            // No events in selected category - Show Newsletter Section
            <div>
              <div className="text-center py-8 max-w-2xl mx-auto">
                <h3 className="text-xl mb-4">
                  {language === 'sr'
                    ? 'Trenutno nema događaja u odabranoj kategoriji'
                    : 'There are currently no events in the selected category'
                  }
                </h3>
                <Button onClick={clearFilters} className="mb-8">
                  {language === 'sr' ? 'Pregledaj sve događaje' : 'Browse All Events'}
                </Button>
              </div>
              
              <div className="rounded-lg overflow-hidden">
                <NewsletterSection hasEvents={false} />
              </div>
            </div>
          ) : (
            // No search results
